...
let getMatch2 (m: Match) =
    (m.Index, (int m.Groups.[1].Value, int m.Groups.[2].Value))
...
let compute2 line =
    ...
    let dos =
        Regex.Matches(line, "do\\(\\)") |> Seq.map (fun x -> x.Index) |> Seq.toArray

    let donts =
        Regex.Matches(line, "don't\\(\\)") |> Seq.map (fun x -> x.Index) |> Seq.toArray

    let filter = doFilter dos donts
    muls |> Seq.filter (fun x -> x |> fst |> filter) |> Seq.sumBy (snd >> multiply)
