...
let getAllPossibleValues map (x, y) =
    ...
    seq {
        if x + 3 < xLength then
            yield [ for i in 0..3 -> map.[y].[x + i] ]

        if x - 3 >= 0 then
            yield [ for i in 0..3 -> map.[y].[x - i] ]

        if y + 3 < yLength then
            yield [ for i in 0..3 -> map.[y + i].[x] ]

        if y - 3 >= 0 then
            yield [ for i in 0..3 -> map.[y - i].[x] ]

        if x + 3 < xLength && y + 3 < yLength then
            yield [ for i in 0..3 -> map.[y + i].[x + i] ]

        if x - 3 >= 0 && y - 3 >= 0 then
            yield [ for i in 0..3 -> map.[y - i].[x - i] ]

        if x + 3 < xLength && y - 3 >= 0 then
            yield [ for i in 0..3 -> map.[y - i].[x + i] ]

        if x - 3 >= 0 && y + 3 < yLength then
            yield [ for i in 0..3 -> map.[y + i].[x - i] ]
    }
...
let getAllPossibleValuesWithPosition map (x, y) =
    ...
    seq {
        if x + 3 < xLength then
            yield ([ for i in 0..3 -> map.[y].[x + i] ], x, y, "right")

        if x - 3 >= 0 then
            yield ([ for i in 0..3 -> map.[y].[x - i] ], x, y, "left")

        if y + 3 < yLength then
            yield ([ for i in 0..3 -> map.[y + i].[x] ], x, y, "down")

        if y - 3 >= 0 then
            yield ([ for i in 0..3 -> map.[y - i].[x] ], x, y, "up")

        if x + 3 < xLength && y + 3 < yLength then
            yield ([ for i in 0..3 -> map.[y + i].[x + i] ], x, y, "right-down")

        if x - 3 >= 0 && y - 3 >= 0 then
            yield ([ for i in 0..3 -> map.[y - i].[x - i] ], x, y, "left-up")

        if x + 3 < xLength && y - 3 >= 0 then
            yield ([ for i in 0..3 -> map.[y - i].[x + i] ], x, y, "right-up")

        if x - 3 >= 0 && y + 3 < yLength then
            yield ([ for i in 0..3 -> map.[y + i].[x - i] ], x, y, "left-down")
    }
...
let getAllPossibleValuesWithPosition2 map (x, y) =
    ...
    seq {
        if x + 2 < xLength && y + 2 < yLength then
            yield ([ for i in 0..2 -> map.[y + i].[x + i] ], x + 1, y + 1, "right-down")

        if x - 2 >= 0 && y - 2 >= 0 then
            yield ([ for i in 0..2 -> map.[y - i].[x - i] ], x - 1, y - 1, "left-up")

        if x + 2 < xLength && y - 2 >= 0 then
            yield ([ for i in 0..2 -> map.[y - i].[x + i] ], x + 1, y - 1, "right-up")

        if x - 2 >= 0 && y + 2 < yLength then
            yield ([ for i in 0..2 -> map.[y + i].[x - i] ], x - 1, y + 1, "left-down")
    }
    |> Seq.map (fun (word, x, y, direction) -> (word |> List.toArray |> String), x, y, direction)
