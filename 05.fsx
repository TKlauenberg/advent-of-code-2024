...
let readPage (page: string)=
    page.Split(',') |> Array.map int
...
let sample1Data = sample1.ByNewLine()|> readData|> Option.get |> fun (a,b) -> compute a b
...
let orderUpdate (rules: (int*int) array) update =
    ...
    let comparer a b=
        if Set.contains (a,b) rulesSet then
            -1
        elif Set.contains (b,a) rulesSet then
            1
        else
            0
    update
...
let sample2Data = sample1.ByNewLine()|> readData|> Option.get |> fun (a,b) -> compute2 a b
